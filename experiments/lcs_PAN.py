...
def tokenize(text, length):
    ...
    tokens = {}
    token = []
    #iterate through the total text (character by character)
    for i in range(0, len(text)): 
        #checks if char in any punct
        if text[i] not in DELETECHARS:
            #char as tokens with their offset giving tuple (offset, char_a)
            token.append((i, text[i]))
        #check if token len meets the len of specified n-gram
        if len(token) == length:
            #join all char to form a n-gram (hence usig second ele of defined tuple)
            ngram = ''.join([x[1].lower() for x in token])
            #see if token already appears in the dict else create nex kev with list as value
            if ngram not in tokens:
                tokens[ngram] = []
            #append token to list with start and end pos of the token in doc
            tokens[ngram].append((token[0][0], token[-1][0]))
            #carry forward with next n-gram
            token = token[1:]
...
class Baseline:
    ...
    def preprocess(self):
        ...
        # susp_fp = codecs.open(self.susp, 'r', 'utf-8')
        self.susp_text = self.susp
        #in preprocessing tokenize susp_text
        self.tokens = tokenize(self.susp_text, LENGTH)
        # susp_fp.close()

        # src_fp = codecs.open(self.src, 'r', 'utf-8')
        self.src_text = self.src
        # src_fp.close()
                        ...
                        for t in self.tokens[ngram]:
                            ...
                            while (start_susp < len(self.susp_text) and
                                ...
                                while (start_src < len(self.src_text) and
                                    ...
                                    start_src = start_src + 1
                            #if same as above d no need to enter new values
                            if (start_src - 1) - token[0][0] > d[0][1] - d[0][0]:
                                d = ((token[0][0], start_src), (t[0], start_susp))
                        detections.append(d)
                        #in order to not go through already visited characters
                        skipto = d[0][1]
